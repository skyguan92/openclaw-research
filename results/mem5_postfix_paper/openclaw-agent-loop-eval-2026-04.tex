...
所以这份报告做的事其实都是在善后。SSE 修完以后 \texttt{claude-code} 和 \texttt{hermes} 的 cache 画像都落到 97.1\% 量级，肉眼基本看不出区别——原报告里"\texttt{claude-code} 独立形态"这条线必须直接撤回。benchmark 也得换：\texttt{astropy-12907} 对三家都 100\% 饱和，区分不出任何东西，于是补进两道 django hard，三家的通过率这才第一次真正分开（6/6 vs 6/6 vs 2/6）。清理过程中还顺手挖出 \texttt{openclaw} adapter 的一个旧 bug——当前轮没有新会话事件时，它会把 \texttt{after\_session["meta"]} 的\emph{累计}计数当本轮 token 用量，我们把这条回退砍掉、并给 6 条受污染的历史记录做了回填。
...
这次两个 bug——上游 shim 的 SSE 解析、adapter 自己的累计 meta 回退——都没让脚本崩，只是在数字里悄悄掺水。我们能发现它们其实靠的是运气：\texttt{pure\_input}=0 这个值太显眼，r04/r05 的 token 数又恰好等于 r03。靠"数字看起来太整齐"反查 bug 不是一种可持续的流程。务实的办法是让遥测本身留点自检的余量。最要紧的一条是把"这一次调用的 usage"和"会话累计 usage"彻底拆成两个字段，不允许回退时混用——我们这次被坑就是这两个值被合并之后，adapter 一走第三层 fallback 就自动撒谎。再加一个 \texttt{has\_new\_activity} 标志也很值得，毕竟"本轮真跑了一遍"和"本轮保留了状态但根本没发过请求"是完全不同的两件事，可现在的记录里看起来一模一样。至于 agent 异常退出，\texttt{error=null} 太宽容了，老老实实写清 \texttt{silent\_exit}/\texttt{panic}/\texttt{timeout} 的成本非常低。下次再遇到"数字怎么这么整齐"的时候，这些字段至少能把排查范围收得紧很多。
...
% ======================= 6. Conclusion =======================
\section{结论}
数据管线自己有 bug 的时候，Agent 间看起来的"显著差异"常常只是管线差异。把解析 bug 和遥测 bug 都修掉之后，两款走 Anthropic 协议的 Agent 在 Kimi 后端上的 cache 画像几乎完全重合——真正把三家拉开的其实是 \texttt{openclaw} 在硬任务上 agent-loop 的稳定性，33\% 的通过率配上两种可重现的失败模式（死循环、过早放弃），说的都是同一件事。对我们下一轮实验的提醒也都挺朴素的：benchmark 别再停在所有人都 100\% 通过的题上；通过率旁边顺手把失败模式的分布也报出来，否则两个 6/6 看起来会像是"没差别"；发稿前留点时间盯着遥测的数字问一句"这组数看起来是不是太整齐了"——我们这次就是这种复盘把两个 bug 揪了出来。
